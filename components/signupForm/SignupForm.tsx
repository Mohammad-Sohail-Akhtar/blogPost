...
const SignupForm = () => {
    ...
    const handleFormSubmit = async(e:FormEvent<HTMLFormElement>) => {
       ...
       if(validate){
            ...
            toast.success('SignUp success',(await response).data)
            router.push('/login')
         ...
         {/* Submit Button */}
          <div>
            <button
              type="submit"
              disabled={isLoading}
              className={`group relative w-full flex justify-center py-3 px-4 border border-transparent rounded-lg text-sm font-bold text-white ${isLoading ? 'bg-blue-400' : 'bg-blue-600 hover:bg-blue-700'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-200`}
            >
              {isLoading ? (
                <>
